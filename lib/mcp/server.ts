...
export const tools = {
	...
	list_components: {
		...
		inputSchema: {
			...
			properties: {
				...
				includeDetails: {
					type: "boolean",
					description:
						"Include full component details (default: false for summary only)",
				},
			},
		},
		handler: (params: { category?: string; includeDetails?: boolean }) => {
			const components = params.category
				? queryComponents({
						category: params.category as QueryOptions["category"],
					}).components
	...
	get_component: {
		name: "get_component",
		description:
			"Get detailed information about a specific component including props, examples, and accessibility",
		inputSchema: {
			type: "object",
			properties: {
				name: {
				...
				format: {
					...
					enum: ["full", "llm"],
					description:
						'Response format: "full" for complete manifest, "llm" for LLM-optimized format',
				},
			},
			required: ["name"],
		},
	...
	query_components: {
		...
		description:
			'Search for components using natural language. Examples: "show status", "collect user input", "display notifications"',
		inputSchema: {
			type: "object",
			properties: {
				query: {
...
export const serverInfo = {
	...
	version: "0.0.1",
	description:
		"AI-first design system - query components, get usage examples, and build UIs",
	capabilities: {
