...
import { loadManifests, getManifest } from './loader';
...
function calculateSemanticScore(
  ...
  for (const keyword of keywords) {
    ...
    // Check useWhen
    if (component.useWhen.some((u) => u.toLowerCase().includes(keyword))) {
      score += 2;
    }
...
function generateSuggestions(components: ComponentManifest[]): string[] {
  ...
  return components.slice(0, 3).map((c) => c.useWhen[0] || c.description);
}
