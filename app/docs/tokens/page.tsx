...
// Opacity scale
const opacityScale = [
	1, 2, 4, 8, 12, 16, 20, 24, 32, 40, 48, 56, 64, 72, 80, 88, 96, 100,
];
...
function ColorSwatch({
	...
	return (
		<button type="button" onClick={copy} className="group text-left">
			...
			>
				<span
					className={`text-[10px] leading-[13px] tracking-[0.12px] ${isLight ? "text-black/48" : "text-white/48"}`}
				>
					{copied ? "Copied!" : value}
				</span>
			</div>
			<div className="mt-2">
				<p className="text-[13px] leading-4 text-white/88 tracking-[0.12px]">
					{name}
				</p>
				{label && (
					<p className="text-[10px] leading-[13px] text-white/48 tracking-[0.12px]">
						{label}
					</p>
				)}
			</div>
