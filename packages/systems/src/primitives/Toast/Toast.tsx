...
const Viewport = forwardRef<HTMLOListElement, ToastViewportProps>((props, ref) => {
...
const Root = forwardRef<HTMLLIElement, ToastRootProps>((props, ref) => {
  ...
  return (
    <ToastContext.Provider value={{ open, onOpenChange: handleOpenChange, duration, type }}>
      <li
        ref={ref}
...
const Action = forwardRef<HTMLButtonElement, ToastActionProps>((props, ref) => {
  ...
  return (
    <button ref={ref} type="button" aria-label={altText} className={className} {...rest}>
      {children}
    </button>
  );
});
