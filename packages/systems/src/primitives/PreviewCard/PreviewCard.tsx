...
function usePreviewCardContext() {
	...
	if (!context) {
		throw new Error(
			"PreviewCard components must be used within PreviewCard.Root",
		);
	}
	return context;
}
...
const Trigger = forwardRef<HTMLAnchorElement, PreviewCardTriggerProps>(
	(props, ref) => {
		const {
			href,
			children,
		...
		return (
			<a
				ref={(node) => {
					(
						context.triggerRef as React.MutableRefObject<HTMLAnchorElement | null>
					).current = node;
...
const Content = forwardRef<HTMLDivElement, PreviewCardContentProps>(
	(props, ref) => {
		const {
			side = "bottom",
			sideOffset = 8,
...
const Arrow = forwardRef<HTMLDivElement, PreviewCardArrowProps>(
	(props, ref) => {
