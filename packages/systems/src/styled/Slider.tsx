...
import React, { forwardRef, useId, useCallback, useRef, useEffect, useState, type HTMLAttributes } from 'react';
      ...
      <div ref={ref} className={`flex flex-col gap-2 ${className}`} {...props}>
        {(label || showValue) && (
          <div className="flex items-center justify-between">
            {label && (
              <label htmlFor={id} className="text-[12px] leading-[15px] font-medium text-white/72 tracking-[0.12px]">
                {label}
              </label>
            )}
            {showValue && (
        ...
        >
          {/* Track */}
          <div className={`relative w-full ${sizes.track} bg-white/8 rounded-full overflow-hidden`}>
