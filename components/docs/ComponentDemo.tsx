  ...
  return (
    <div className="flex flex-col gap-4 w-full max-w-xs">
      <StyledInput
        ...
        onChange={(val) => {
          setValue(val);
          setError(val.length > 0 && val.length < 3 ? 'Must be at least 3 characters' : '');
        }}
        error={error}
    ...
    <div className="flex flex-col gap-3">
      {['option1', 'option2', 'option3'].map((option) => (
        <label key={option} className="flex items-center gap-2 cursor-pointer">
          ...
          />
          <span className="text-white/72 capitalize">{option.replace('option', 'Option ')}</span>
        </label>
      ))}
    </div>
  );
...
function RadioGroupDemo() {
  ...
  return (
    <RadioGroup.Root value={value} onChange={setValue} className="flex flex-col gap-3">
...
function ProgressDemo() {
  ...
  return (
    <div className="w-full max-w-xs space-y-4">
      <Progress.Root value={progress} className="relative w-full h-2 bg-white/8 rounded-full overflow-hidden">
        <Progress.Indicator
...
function AccordionDemo() {
  return (
    <Accordion.Root type="single" className="w-full max-w-md border border-white/8 rounded-lg overflow-hidden">
      <Accordion.Item value="item-1" className="border-b border-white/8 last:border-b-0">
      ...
      <Accordion.Item value="item-2" className="border-b border-white/8 last:border-b-0">
        <Accordion.Trigger className="w-full px-4 py-3 text-left text-white hover:bg-white/4 flex justify-between items-center">
          Is it styled?
          <span className="text-white/48">+</span>
        </Accordion.Trigger>
        <Accordion.Content className="px-4 py-3 text-white/72 bg-white/4">
          No. It&apos;s unstyled by default, giving you freedom over the look and feel.
        </Accordion.Content>
      </Accordion.Item>
      <Accordion.Item value="item-3" className="border-b border-white/8 last:border-b-0">
        <Accordion.Trigger className="w-full px-4 py-3 text-left text-white hover:bg-white/4 flex justify-between items-center">
          Can it be animated?
          <span className="text-white/48">+</span>
        </Accordion.Trigger>
...
function ToggleGroupDemo() {
  return (
    <ToggleGroup.Root type="single" defaultValue="center" className="flex border border-white/8 rounded-lg overflow-hidden">
      <ToggleGroup.Item
        value="left"
    ...
    <div>
      ...
      <Toast.Provider>
        {showToast && (
          ...
          >
            <Toast.Title className="text-white font-medium">Success!</Toast.Title>
...
function ScrollAreaDemo() {
  return (
    <ScrollArea.Root className="w-full max-w-xs h-48 border border-white/8 rounded-lg">
      ...
      <ScrollArea.Scrollbar orientation="vertical" className="w-2 bg-white/4 rounded-full">
  ...
  return (
    <div className="w-full max-w-xs space-y-4">
      <div>
        ...
        <Meter.Root value={75} className="relative w-full h-2 bg-white/8 rounded-full overflow-hidden">
          <Meter.Track>
            <Meter.Indicator className="h-full bg-amber-500" style={{ width: '75%' }} />
          </Meter.Track>
        </Meter.Root>
      </div>
      <div>
        <div className="text-white/48 text-sm mb-1">Battery (90%)</div>
        <Meter.Root value={90} className="relative w-full h-2 bg-white/8 rounded-full overflow-hidden">
          <Meter.Track>
            <Meter.Indicator className="h-full bg-green-500" style={{ width: '90%' }} />
          </Meter.Track>
        </Meter.Root>
      </div>
      <div>
        <div className="text-white/48 text-sm mb-1">Memory (25%)</div>
        <Meter.Root value={25} className="relative w-full h-2 bg-white/8 rounded-full overflow-hidden">
          <Meter.Track>
            <Meter.Indicator className="h-full bg-sky-500" style={{ width: '25%' }} />
          </Meter.Track>
        </Meter.Root>
      </div>
    </div>
...
function NumberFieldDemo() {
  ...
  return (
    <NumberField.Root value={value} onValueChange={setValue} min={0} max={10} className="w-full max-w-[160px]">
      <NumberField.Label className="text-white/48 text-sm mb-1 block">Quantity</NumberField.Label>
...
function FieldDemo() {
  ...
  return (
    <div className="w-full max-w-xs space-y-4">
      <Field.Root>
        <Field.Label className="text-white text-sm mb-1 block">Email</Field.Label>
        <Field.Control>
          <Input
            type="email"
            placeholder="Enter your email"
      ...
      <Field.Root hasError={hasError}>
        <Field.Label className="text-white text-sm mb-1 block">Password</Field.Label>
        <Field.Control>
          <Input
            type="password"
            placeholder="Enter password"
            onChange={(e) => setHasError(e.target.value.length > 0 && e.target.value.length < 6)}
...
function ComboboxDemo() {
  ...
  return (
    <Combobox.Root value={value} onValueChange={setValue} className="w-full max-w-xs relative">
...
function CheckboxGroupDemo() {
  ...
  return (
    <CheckboxGroup.Root value={values} onValueChange={setValues} className="space-y-3">
      <CheckboxGroup.Label className="text-white font-medium">Select frameworks</CheckboxGroup.Label>
      {['react', 'vue', 'svelte', 'angular'].map((framework) => (
        <div key={framework} className="flex items-center gap-2">
          ...
          />
          <CheckboxGroup.ItemLabel htmlFor={framework} className="text-white/72 capitalize cursor-pointer">
            {framework}
...
function BadgeDemo() {
  return (
    <div className="flex flex-wrap gap-3">
      ...
      <StyledBadge size="sm" variant="default">Small</StyledBadge>
      <StyledBadge size="sm" variant="accent">Pro</StyledBadge>
    </div>
  );
}
  ...
  return (
    <StyledCard padding="none" className="w-full max-w-sm">
      <StyledCard.Header>
        ...
        <StyledCard.Description>This is a styled card component with header, content, and footer.</StyledCard.Description>
      </StyledCard.Header>
      <StyledCard.Content>
        <p className="text-sm text-white/72">Card content goes here. You can add any components inside.</p>
      </StyledCard.Content>
      <StyledCard.Footer>
        <StyledButton variant="ghost" size="sm">Cancel</StyledButton>
        <StyledButton variant="primary" size="sm">Save</StyledButton>
      </StyledCard.Footer>
    </StyledCard>
  );
}
